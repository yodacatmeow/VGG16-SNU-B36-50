"""
Description
    # VGG16_adap for supervised representation learning; 39 categories
References
...
class vgg16:
    ...
    def __init__(self, imgs, img_mean, weights=None, sess=None, bn=False, bn_is_training=True):
        self.imgs = imgs
    ...
    def fc_layers(self):
        ...
        with tf.name_scope('fc4') as scope:
            self.fc4w = tf.Variable(tf.truncated_normal([1000, 39], dtype=tf.float32, stddev=1e-1), name='weights')
            self.fc4b = tf.Variable(tf.constant(1.0, shape=[39], dtype=tf.float32), trainable=True, name='biases')
